...
describe('useScreenshot hook', () => {
  ...
  test('Should initialize with null screenshot', () => {
    const { result } = renderHook(() => useScreenshot());
    expect(result.current.screenshot).toBeNull();
  });

  test('Should clean up by revoking object URL when unmounted', () => {
  ...
  test('should take a screenshot and return blob and URL', async () => {
    // Mock the video element and canvas with proper types
    const mockBlob = new Blob(['test'], { type: 'image/png' });
    const toBlobSpy = jest.fn((callback: (blob: Blob | null) => void) => callback(mockBlob));
    ...
    expect(screenshotResult).toEqual({ blob: mockBlob, screenshotUrl: 'mockedUrl' });
    expect(navigator.mediaDevices.getDisplayMedia).toHaveBeenCalled();
    expect(mockStream.getTracks).toHaveBeenCalled();
  });


  test('Should handle error during screenshot capture', async () => {
    (navigator.mediaDevices.getDisplayMedia as jest.Mock).mockRejectedValue(new Error('Permission denied'));

    const { result } = renderHook(() => useScreenshot());

    await expect(result.current.takeTradingViewScreenshot()).rejects.toThrow('Permission denied');
  });
});
